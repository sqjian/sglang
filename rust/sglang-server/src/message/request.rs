...
pub struct GenerateBody {
    ...
    /// PD-disaggregation routing fields injected by the router. Batch elements
    /// are nullable because an absent bootstrap port defers to the scheduler's
    ...
    /// DP routing hints are scalar on Python's `GenerateReqInput`, including
    /// for a batch, and therefore copy into every fanned-out request.
    #[serde(default)]
...
impl GenerateBody {
    ...
    pub fn into_requests(self) -> Result<(Vec<GenerateRequest>, bool), Error> {
        ...
        // Python `_normalize_bootstrap_params`: scalar columns broadcast,
        // except a scalar room advances by one for each item so every P↔D pair
        // gets a distinct key. `fan_out`'s two option layers represent an
        // absent field and an explicit null list element; both mean unset once
        // the per-request column is built.
        let bootstrap_hosts =
            flatten_nullable_column(fan_out(bootstrap_host, n, "bootstrap_host")?);
        let bootstrap_ports =
            flatten_nullable_column(fan_out(bootstrap_port, n, "bootstrap_port")?);
        let bootstrap_rooms = match bootstrap_room {
            // Rooms generated by the supported routers are in `[0, 2^63)`.
            // Wrapping preserves distinct adjacent values at the integer edge;
            // saturating would collapse multiple requests onto one room.
            Some(OneOrMany::One(Some(room))) => (0..n)
                .map(|index| Some(room.wrapping_add(index as i64)))
                .collect(),
            other => flatten_nullable_column(fan_out(other, n, "bootstrap_room")?),
        };
        let bootstrap_pair_keys =
            flatten_nullable_column(fan_out(bootstrap_pair_key, n, "bootstrap_pair_key")?);
        let decode_tp_sizes =
            flatten_nullable_column(fan_out(decode_tp_size, n, "decode_tp_size")?);
...
pub struct GenerateRequest {
    ...
    pub return_text_in_logprobs: Option<bool>,
    /// PD bootstrap routing forwarded to the Python scheduler. A missing port
    /// is resolved there from the launch configuration; a missing room is a
    /// client error on a real PD request.
...
/// Collapse the nullable field and nullable element layers produced by
/// `fan_out`; downstream both an absent field and an explicit null mean unset.
fn flatten_nullable_column<T>(column: Vec<Option<Option<T>>>) -> Vec<Option<T>> {
...
mod tests {
    ...
    /// PD bootstrap fields follow Python `_normalize_bootstrap_params`: scalars
    /// broadcast, except a scalar room advances once per batch item.
    #[test]
    fn bootstrap_fields_broadcast_and_advance_room() {
        let (requests, is_batch) = requests(
            r#"{
                "text": ["a", "b"],
                "bootstrap_host": "prefill",
                "bootstrap_port": 8998,
                "bootstrap_room": 7,
                "bootstrap_pair_key": "pair",
                "decode_tp_size": 2,
                "routed_dp_rank": 3,
        ...
        for (index, request) in requests.iter().enumerate() {
            assert_eq!(request.bootstrap_host.as_deref(), Some("prefill"));
            assert_eq!(request.bootstrap_port, Some(8998));
            assert_eq!(request.bootstrap_room, Some(7 + index as i64));
            assert_eq!(request.bootstrap_pair_key.as_deref(), Some("pair"));
            assert_eq!(request.decode_tp_size, Some(2));
            assert_eq!(request.routed_dp_rank, Some(3));
            assert_eq!(request.disagg_prefill_dp_rank, Some(4));
        }
    }

    /// Router-provided per-item lists may contain nulls. Each list is aligned
    /// to the request batch and a mismatched list is rejected.
    #[test]
    fn bootstrap_fields_accept_nullable_lists_and_validate_lengths() {
        let (reqs, _) = requests(
            r#"{
                "text": ["a", "b"],
                "bootstrap_host": [null, "prefill"],
                "bootstrap_port": [null, 8998],
                "bootstrap_room": [null, 20],
                "bootstrap_pair_key": [null, "pair"],
                "decode_tp_size": [null, 2]
            }"#,
        )
        .unwrap();

        assert_eq!(reqs[0].bootstrap_host, None);
        assert_eq!(reqs[0].bootstrap_port, None);
        assert_eq!(reqs[0].bootstrap_room, None);
        assert_eq!(reqs[0].bootstrap_pair_key, None);
        assert_eq!(reqs[0].decode_tp_size, None);
        assert_eq!(reqs[1].bootstrap_host.as_deref(), Some("prefill"));
        assert_eq!(reqs[1].bootstrap_port, Some(8998));
        assert_eq!(reqs[1].bootstrap_room, Some(20));
        assert_eq!(reqs[1].bootstrap_pair_key.as_deref(), Some("pair"));
        assert_eq!(reqs[1].decode_tp_size, Some(2));

        let error = requests(r#"{"text": ["a", "b"], "bootstrap_room": [1, 2, 3]}"#).unwrap_err();
        assert!(error.to_string().contains("bootstrap_room"), "{error}");
    }

    /// The PD warmup shape is scalar and pre-tokenized; it must survive the
    /// same request parser used by external `/generate` traffic.
    #[test]
    fn accepts_pd_warmup_payload() {
        let (requests, is_batch) = requests(
            r#"{
                "sampling_params": {
                    "temperature": 0.0,
        ...
        assert!(!is_batch);
        assert_eq!(requests[0].bootstrap_host.as_deref(), Some("2.2.2.2"));
        assert_eq!(requests[0].bootstrap_room, Some(0));
        assert_eq!(requests[0].routed_dp_rank, Some(0));
    }
