...
def get_target_cache_loc(
    ...
    accept_len_all = tl.load(num_correct_drafts + safe_bs_offset, mask=bs_mask)
...
def filter_finished_cache_loc_kernel(
    ...
    num_correct_drafts_all = tl.load(
        num_correct_drafts + safe_bs_offset, mask=bs_mask
    )
    old_start = tl.sum(num_correct_drafts_all) + bid

    num_accept_tokens_filter_all = tl.load(
        num_accept_tokens_filter + safe_bs_offset, mask=bs_mask
    )
