...
class TestPrefillBootstrapAdmission(unittest.TestCase):
    ...
    def test_mooncake_receiver_abort_notifies_prefill_status_before_metadata(self):
        ...
        self.assertEqual(receiver.kv_mgr.failures, [(17, "decode aborted before metadata")])
        ...
        self.assertTrue(all(msg[0] == MooncakeKVManager.DECODE_STATUS_HEADER for msg in fake_socket.messages))
        self.assertTrue(all(msg[1] == b"17" for msg in fake_socket.messages))
        self.assertTrue(all(msg[2] == str(KVPoll.Failed).encode("ascii") for msg in fake_socket.messages))
        self.assertTrue(all(msg[3] == b"decode aborted before metadata" for msg in fake_socket.messages))
