...
def _normalize_tool_content(role: str, content: Any) -> Any:
    if role != "tool" or not isinstance(content, list):
        return content
    if all((isinstance(part, dict) and part.get("type") == "text") or isinstance(part, str) for part in content):
        return " ".join(part.get("text", "") if isinstance(part, dict) else part for part in content)
    return content


def _normalize_messages_for_tokenizer(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for message in normalized:
        ...
        if message.get("content") is None:
            message["content"] = ""
        message["content"] = _normalize_tool_content(message["role"], message.get("content"))
        if message["role"] != "assistant" or not isinstance(message.get("tool_calls"), list):
            continue
        for tool_call in message["tool_calls"]:
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    raise ValueError("OpenAI dataset tool call arguments are invalid JSON") from error
    return normalized


def _merge_request_body(record_body: Dict[str, Any], global_body: Dict[str, Any]) -> Dict[str, Any]:
...
def _count_prompt_tokens(
    ...
    try:
        input_ids = tokenizer.apply_chat_template(normalized_messages, **template_kwargs)
    except Exception:
        if not tools:
            raise
    ...
    with open(dataset_path, "r") as f:
        for line in f:
            ...
            if not isinstance(messages, list) or not messages:
                continue
            output_len = fixed_output_len if fixed_output_len is not None else data.get(
                "max_tokens", data.get("max_completion_tokens", 256)
            )
            if isinstance(output_len, bool) or not isinstance(output_len, int) or output_len < 1:
                raise ValueError("OpenAI dataset output length must be a positive integer")
            excluded_fields = {"messages", "max_tokens", "max_completion_tokens", "model"}
            record_body = {key: value for key, value in data.items() if key not in excluded_fields}
