"""Multi-GPU FlashMLA-KV DCP correctness check for SM90.

Run on one H20 node with DCP size 2, 4, or 8, for example::
...
def main() -> None:
    ...
    _run_case(
        case_name=f"FlashMLA KV DCP{world_size} balanced SM90",
        q=q,
        global_kv=global_kv,
        topk_indices=balanced_indices,
    )
    _run_case(
        case_name=f"FlashMLA KV DCP{world_size} zero-local-KV SM90",
        q=q,
        global_kv=global_kv,
